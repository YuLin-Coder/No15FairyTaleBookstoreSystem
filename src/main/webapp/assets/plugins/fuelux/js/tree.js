...
(function (factory) {
	...
	var Tree = function Tree(element, options) {
		...
		this.options = $.extend({}, $.fn.treeFx.defaults, options);
			...
			this.options.dataSource(treeData ? treeData : {}, function (items) {
				$.each(items.data, function (index, value) {
					...
					if (value.type === 'folder') {
						...
						$entity.data(value);
						$entity.find('.tree-branch-header > .tree-label').html(value.text || value.name);
						...
						$entity.find(' .tree-label').html(value.text || value.name);
						$entity.data(value);
					...
					$.each(attr, function (key, value) {
						switch (key) {
							...
							case 'id':
								...
								$entity.find('.tree-branch-header > .tree-label').attr('id', value + '-label');
								break;
	...
	$.fn.treeFx = function tree(option) {
	...
	$.fn.treeFx.Constructor = Tree;

	$.fn.treeFx.noConflict = function () {
		$.fn.treeFx = old;
		return this;
	};
