var Index = function () {
    ...
    return {
       ...
       dataTableAjax:function(sUrl,aoData,fnCallback,methodType,extParamFunc){
    	   var jsonHaveSort = JSON.stringify(aoData).indexOf('sort');
		   if(!methodType){
			 methodType="GET";
		   }
		   var count=0;
		   ...
		   var rows=10;
       	   ...
       	   aoData.push( { "name": "rows", "value":rows } );
       	   aoData.push( { "name": "page", "value":parseInt(page/rows)+1 } );
       	   aoData.push( { "name": "order", "value": "desc"} );
       	   if(jsonHaveSort==-1){
       		 aoData.push( { "name": "sort", "value":"id" } );
       	   }
       	   
	       //添加额外参数
       	   if(typeof(extParamFunc)!= 'undefined' && extParamFunc != null){
       		var extParamArr = extParamFunc();
       		for(var i=0;i<extParamArr.length;i++){
       			aoData.push(extParamArr[i]);
       		}
			  ...
			  "error": function (xhr, error, thrown) {
				console.log("load dataTable error..");
			  }
		  } );
       }
